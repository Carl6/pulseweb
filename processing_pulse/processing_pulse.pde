...
void setup() {
  ...
  frameRate(100);
  font = loadFont("Phenomena-Regular-40.vlw");
  textFont(font);
  textAlign(CENTER);
  rectMode(CENTER);
  ellipseMode(CENTER);
...
// Map mouseY from 0.0 to 1.0 for amplitude
  amp=map(BPM, 0, height, 1.0, 0.0);
  sine.amp(amp);
  
  // Map mouseX from 20Hz to 1000Hz for frequency  
  freq=map(BPM, 0, width, 80.0, 1000.0);
  sine.freq(freq);
  
  // Map mouseX from -1.0 to 1.0 for left to right 
  pos=map(BPM, 0, width, -1.0, 1.0);
  sine.pan(pos);
...
void drawPulseWaveform(){
  ...
  stroke(#0097AC);                               // red is a good color for the pulse waveform
  noFill();
...
// then limit and scale the BPM value
 ...
 stroke(#0097AC);                          // color of heart rate graph
 strokeWeight(3);                          // thicker line is easier to read
 noFill();
 beginShape();
...
void listAvailablePorts(){
  ...
  for(int i=serialPorts.length-1; i>=0; i--){
    button[i] = new Radio(xPos, 95+(yPos*20),12,color(#0097AC),color(#9ACBD1),color(#0097AC),i,button);
